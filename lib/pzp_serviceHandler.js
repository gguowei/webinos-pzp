...
var PzpServiceHandler = function () {
    ...
    function loadModules() {
        ...
        // webinos-android API path: webinos-android/node_modules/webinos-api-*
        if(PzpCommon.os.platform().toLowerCase() == "android") {
            ownModules = PzpCommon.wUtil.webinosService.checkForWebinosModules(PzpCommon.path.join(__dirname, "../..")).concat(ownModules);
            for(var i = 0; i < ownModules.length; ++i) {
                logger.log("DEBUG: -- loaded module -- " + ownModules[i].name);
                for(var j = i+1; j < ownModules.length; ++j)
                    if(ownModules[i].name === ownModules[j].name)
                        ownModules.splice(j--, 1);
            }
        }
        ...
        // DEBUG: need to check how path and modules are used.
        PzpCommon.wUtil.webinosService.createWebinosJS(nodeModulesPath, ownModules);
            // create initial webinosJSPzp
    }
